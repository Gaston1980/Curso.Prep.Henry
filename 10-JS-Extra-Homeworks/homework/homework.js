...
function numberOfCharacters(string) {
  //La función recibe un string. Recorre el string y 
...
for( var i = 0; i < s.length; i++) {
  if( s[i] === s[i].toUpperCase()){
    arrayMayusculas.push(s[i])
  } else {
    arrayMinusculas.push(s[i])
  }
}
...
function asAmirror(str) {
 ...
 var newArray = []
  for( var i = 0; i < conver.length; i++) {
    newArray.push(conver[i].split("").reverse().join(""))
  }
...
function deleteAbc(cadena){
  ...
  //Escribe tu código aquí
  var conver = cadena.split("")
  var sinABC = []
  for(i = 0; i < conver.length; i++) {
    if( conver[i] === "a" || conver[i] === "b" || conver[i] === "c") {
      continue;
    } else {
      sinABC.push(conver[i])
    }
  }
  return sinABC.join("")
}
...
function sortArray(arr) {
  ...
  //Escribe tu código aquí
  var arrayCreciente = arr.sort(function(a,b) {
